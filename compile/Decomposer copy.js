...
class Decomposer
{
    ...
    static joinFromString(string, scope)
    {
        ...
        function treatOperator(op)
        {
            ...
            {
                node.operator = op;
                node.nodeA = that.nodeFromString(nodeA, scope);
                node.nodeB = that.nodeFromString(nodeB, scope);
            }else{
                node.type = 'declare';
                node.var = nodeA;
                node.value = that.nodeFromString(nodeB, scope);
            }
        ...
        function tryDouble(op)
        {
            ...
            const run1 = bs.readTill(op.charAt(0));

            if(!run1.found) return false;

            const run2 = bs.readTill(op.charAt(1));
